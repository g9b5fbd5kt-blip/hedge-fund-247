...
CONFIG = {
    'version': '5.1.0', 'optimizations': 700, 'crypto_focus': 0.70, 'scan_interval': 8,
    'heartbeat_minutes': 3, 'max_daily_loss': 20, 'max_trades_per_day': 40, 'min_notional': 10.0,
    'buy_score_min': 58, 'sell_score_max': 25, 'max_positions': 12, 'risk_per_trade': 0.015,
    'max_portfolio_risk': 0.06, 'kelly_fraction': 0.25, 'correlation_limit': 0.7,
    'spread_limit': 0.005, 'min_volume_24h': 1000000,
    ...
    'haptic_enabled': True, 'biometric_gate': True, 'emergency_stop': True,
    'websocket_binary': True, 'cache_ttl': 5,
}

CORE_PHRASES = ["checkin stocks, not flipping rocks","real bosses don't talk they just sit back and listen","First you get the money then you get the power","get up and get some money","bag secured","paper chaser","clean money over here","generational wealth","Stack that paper up and then make boss moves","countin' dividends, not sheep","money talks, we let it speak","charts up, stress down","real ones trade, phones on mute","cash in hand, plan in motion","market makers, not heart breakers","wealth mode activated","paper stack, never look back","boss moves only","silent profits, loud results","grinding charts, not playing parts","money printer go brr, we go further","position heavy, pockets ready","buy low, live high","sell high, stay fly","capital gains, zero pains","assets up, liabilities shut","passive income, active vision","portfolio fat, ego flat","money work, we don't work","cash flow king","dividend drip, ownership","equity built, debt killed","net worth up, net stress down","financial free, mentally wealthy","money moves made in silence","wealth whispers, broke shouts","capital compounds, excuses don't","profits talk, losses walk","trading desk, not 9 to 5","market close, we still open","after hours, we got power","pre market prep, post market profit","gap up, we don't give up","gap down, we buy the crown","volume spike, we take flight","liquidity pool, we make the rules","order flow, we run the show","price action, boss reaction","candles green, portfolio clean","wicks long, money strong","breakout confirmed, capital earned","trend riding, wealth providing","momentum building, empire building","legacy creating, never debating"]
...
conn.executescript('''
CREATE TABLE IF NOT EXISTS trades (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT NOT NULL, symbol TEXT NOT NULL, side TEXT NOT NULL, quantity REAL NOT NULL, price REAL NOT NULL, notional REAL NOT NULL, pnl REAL DEFAULT 0, reason TEXT, phrase TEXT, hold_days INTEGER DEFAULT 0, tax_status TEXT, score INTEGER, rsi REAL, tier INTEGER, slippage REAL DEFAULT 0, commission REAL DEFAULT 0, market_impact REAL DEFAULT 0, fill_time_ms INTEGER DEFAULT 0, kelly_size REAL DEFAULT 0);
CREATE TABLE IF NOT EXISTS equity_history (timestamp TEXT PRIMARY KEY, equity REAL NOT NULL, cash REAL NOT NULL, positions_count INTEGER, daily_pnl REAL DEFAULT 0, tier INTEGER, sharpe REAL DEFAULT 0, sortino REAL DEFAULT 0, max_dd REAL DEFAULT 0, portfolio_heat REAL DEFAULT 0);
CREATE TABLE IF NOT EXISTS phrase_usage (timestamp TEXT, phrase TEXT, phrase_type TEXT, context TEXT);
CREATE TABLE IF NOT EXISTS scan_log (timestamp TEXT, symbol TEXT, score INTEGER, signal TEXT, reason TEXT, price REAL, rsi REAL, volume_ratio REAL, confidence REAL DEFAULT 0, regime TEXT, spread REAL DEFAULT 0, liquidity REAL DEFAULT 0);
CREATE TABLE IF NOT EXISTS correlations (symbol1 TEXT, symbol2 TEXT, correlation REAL, timestamp TEXT, PRIMARY KEY (symbol1, symbol2));
''')
...
class BeastEngine:
    ...
    async def send_telegram(self, text, silent=False, is_daily=False):
        ...
        try:
            ...
            full_text = f"📊 **Daily Summary v5.1**\n\n{core_phrase}\n\n{text}" if is_daily else f"{core_phrase}\n\n{text}"
            await tg.send_message(chat_id=TG_CHAT, text=full_text, parse_mode='Markdown', disable_notification=silent, disable_web_page_preview=True)
    ...
    async def broadcast(self, data):
        if self.ws_clients:
            msg = json.dumps(data).encode() if CONFIG['websocket_binary'] else json.dumps(data)
    ...
    async def fetch_data(self, symbol, timeframe='1h'):
        ...
        if cache_key in self.data_cache and time.time() - self.cache_timestamps[cache_key] < CONFIG['cache_ttl']:
    ...
    async def execute_trade(self, symbol, side, analysis, tier):
        try:
            ...
            try:
                conn.execute("INSERT INTO trades (timestamp, symbol, side, quantity, price, notional, reason, phrase, score, rsi, tier, fill_time_ms, kelly_size) VALUES (?,?,?,?,?,?,?,?,?)", (datetime.now().isoformat(), symbol, side, qty, price, qty * price, analysis['reason'], phrase, analysis['score'], analysis['rsi'], tier, fill_time, analysis['kelly_size']))
            except: pass
            self.trades_today += 1
    ...
    async def beast_scan(self):
        ...
        for symbol in symbols[:14]:
            ...
            if analysis:
                ...
                try:
                    conn.execute("INSERT INTO scan_log VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (datetime.now().isoformat(), symbol, analysis['score'], analysis['signal'], analysis['reason'], analysis['price'], analysis['rsi'], analysis['vol_ratio'], analysis['confidence'], self.market_regime, analysis['spread'], analysis['liquidity']))
                except: pass
...
async def health(request):
    return web.json_response({'status': 'online', 'version': '5.1.0'})
...
async def api_chart(request):
    ...
    candles = [{'time': int(row['timestamp'].timestamp()), 'open': float(row['open']), 'high': float(row['high']), 'low': float(row['low']), 'close': float(row['close'])} for _, row in df.iterrows()]
